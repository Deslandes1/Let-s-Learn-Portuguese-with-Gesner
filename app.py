...
tema = temas[lesson_number-1]
st.markdown(f"## Lição {lesson_number}: {tema}")
tabs = st.tabs(["💬 Conversas", "📚 Vocabulário", "📖 Gramática", "🎧 Pronúncia", "❓ Questionário"])

with tabs[0]:
    st.text(f"A: Olá! Como vai?\nB: Tudo bem! Estou estudando {tema}.")
    reproducir_audio(f"Olá! Como vai? Tudo bem! Estou estudando {tema}.", f"aud_{lesson_number}")

# (Rest of your UI logic follows here...)
